...
public final class IDCardUtil {
    ...
    public static boolean checkLength(String code) {
        ...
        if (length == 9) {
            ...
            _codeError = "错误：输入的证件号不是9位的";
            return false;
        }
    }

    //验证台胞证位数,>6位
    public static boolean checkMTPSLength(String code) {
        int length = code.length();
        if (length > 6) {
            return true;
        } else {
            _codeError = "错误：输入的证件号不对";
            return false;
        }
    }

    //验证港澳通行证位数,11位
    public static boolean checkPSPLength(String code) {
        int length = code.length();
        if (length == 11) {
            return true;
        } else {
            _codeError = "错误：输入的证件号不是11位的";
            return false;
        }
    }
...
//            Element child=  _areaCodeElement.getChild("_"+areaCode);  
    ...
    //验证除了最后位其他的是否包含字母
    public boolean containsPassNumber(String code) {
        String str = "";
        if (code.length() == 9) {
            str = code.substring(1, 9);
        }
        char[] ch = str.toCharArray();
        for (int i = 0; i < ch.length; i++) {
            if (!(ch[i] >= '0' && ch[i] <= '9')) {
                _codeError = "错误：输入的证件号第" + (i + 1) + "位包含字母";
                return false;
            }
        }
        return true;
    ...
    //验证除了最后位其他的是否包含字母
    public boolean containsMTPSNumber(String code) {
        String str = "";
        if (code.length() == 13) {
    ...
    //验证除了最后位其他的是否包含字母
    public boolean containsPSPNumber(String code) {
        String str = "";
        if (code.length() == 11) {
            str = code.substring(1, 10);
        }
        char[] ch = str.toCharArray();
        for (int i = 0; i < ch.length; i++) {
            if (!(ch[i] >= '0' && ch[i] <= '9')) {
    ...
    //验证第一位是否为字母
    public boolean checkMTPFirst(String ID) {
        String first = ID.substring(10,13);
        String str = "(B)";
        if (!first.equals(str)) {
            _codeError = "错误：输入的证件号错误";
            return false;
        }
        return true;
    }
    ...
    //验证第一位是否为字母
    public boolean checkFirst(String ID) {
    ...
    //验证第一位是否为字母
    public boolean checkPSPFirst(String ID) {
    ...
    public boolean checkPassport(String ID) {

        //验证护照位数,9位，一个字母加8位数字
        if (!checkLength(ID)) {
            return false;
        }
        if (!containsPassNumber(ID)) {
            return false;
        }
        if (!checkFirst(ID)) {
            return false;
    ...
    public boolean checkMTPS(String ID) {
        //验证台胞证位数,>6 位数字加
        if (!checkMTPSLength(ID)) {
            return false;
        }
       ...
       /* if (!checkMTPFirst(ID)) {
        ...
        if (!containsMTPSNumber(ID)) {
            return false;
        }*/
        
        /**
         * 台胞证是否符合身份号规则，防止用身份证进行其他证件类型的是名认证
         */
        if(this.verify(ID)){
        	return false;
        }
    ...
    public boolean checkPSP(String ID) {
        //验证港澳通行证位数,11位
        if (!checkPSPLength(ID)) {
            return false;
        }
...
//            }  
    ...
    public static void main(String[] args) {
        String idc = "";
        boolean ret = new IDCardUtil().verify(idc);
        //boolean ret = new IDCardUtil().checkPSP(idc);
