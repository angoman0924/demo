...
/**
 * web工具
 * Created by:ruben Date:16/7/26 Time:下午3:04
 */
public class WebUtils {
     ...
     * @return
     */
    public static <T,D> Result<PageResult<T>> pageDTOConvert2PageResult(PageInfo<D> pageInfo,Class<T> clazz){
        if (!ObjectUtils.isNull(pageInfo) && !ObjectUtils.isNull(pageInfo.getList()) && pageInfo.getList().size() > 0) {
            String s = JSON.toJSONString(pageInfo.getList());
            List<T> list = JSON.parseArray(s, clazz);
     ...
     * @return
     */
    public static <D,T> List<D> listDTOConvert2ListResult(List<T> listEntity,Class<D> clazz){
