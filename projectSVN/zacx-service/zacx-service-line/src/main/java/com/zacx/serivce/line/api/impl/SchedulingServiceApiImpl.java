...
public class SchedulingServiceApiImpl implements SchedulingServiceApi {
    ...
    @Override
    public SchedulingDTO getSchedulingByCurrentDriver(Integer driverId) {
        logger.info("SchedulingServiceApi.getSchedulingByCurrentDriver,params:{}",driverId);
        LSchedulingExample example=new LSchedulingExample();
        example.createCriteria().andDriverIdEqualTo(driverId).andIsSignEqualTo(true);
        List<LScheduling> list = schedulingService.getSchedulingListByExample(example);
