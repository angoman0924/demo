...
public class OrderChangeLocServiceImpl extends BaseOrderLocServiceImpl implements OrderChangeLocService {
    ...
    public Boolean doChange(ChangeOrderDTO changeOrderDTO) {
        ...
        if (OrderOperateTypeEnum.CANCEL.equals(changeOrderDTO.getOperateType())) {
            ...
            userOperateOrderLog(orderEntity, oldStatus.get(), "取消订单", "");
        } else if (OrderOperateTypeEnum.REFUND.equals(changeOrderDTO.getOperateType())) {
            refundOrder(orderEntity);
            userOperateOrderLog(orderEntity, oldStatus.get(), "申请退款", "");
        }
    ...
    public void finishScheduling(Integer schedulingId) throws OrderServiceException {
        if(schedulingId == null || schedulingId <= 0){
            throw new OrderServiceException(Code.ERROR_ARGUMENT, "请提供调度信息");
        }

        //备份历史状态数据：待司机结束行程时，未验票的车票状态，会被系统设置为过期
        OTicketExample overdueTicketExample = new OTicketExample();
        overdueTicketExample.createCriteria().andSchedulingIdEqualTo(schedulingId)
                .andStatusIn(Arrays.asList(TicketStatusEnum.NEW.getCode()));
        OTicket uptTicket = new OTicket();
        uptTicket.setStatus(TicketStatusEnum.OVERDUE.getCode());
        uptTicket.setModifyAt(new Date());
        uptTicket.setModifyBy("system");
        oTicketMapper.updateByExampleSelective(uptTicket, overdueTicketExample);

        //查询出正常验票的数据，得到对应的订单ID
        OTicketExample verifiedTicketExample = new OTicketExample();
        verifiedTicketExample.createCriteria().andSchedulingIdEqualTo(schedulingId)
                .andStatusIn(Arrays.asList(TicketStatusEnum.VERIFIED.getCode()));
        List<OTicket> verifiedTicketList = oTicketMapper.selectByExample(verifiedTicketExample);
        List<Integer> verifiedOrderIds = verifiedTicketList.stream().map(o->o.getOrderId()).distinct().collect(Collectors.toList());

        //备份历史状态数据：处理付款后正常运营的订单
        OOrderExample orderBaseExample = new OOrderExample();
        OOrderExample.Criteria orderBaseCriteria = orderBaseExample.createCriteria();
        orderBaseCriteria.andSchedulingIdEqualTo(schedulingId)
                .andStatusIn(Arrays.asList(OrderStatusEnum.PAID.getCode()));
        List<OOrder> oldOrderList = oOrderMapper.selectByExample(orderBaseExample);

        //将已经验票的改为“已完成”
        orderBaseCriteria.andIdIn(verifiedOrderIds);
        OOrder uptFinishOrder = new OOrder();
        uptFinishOrder.setStatus(OrderStatusEnum.FINISH.getCode());
        uptFinishOrder.setModifyAt(new Date());
        uptFinishOrder.setModifyBy("system");
        oOrderMapper.updateByExampleSelective(uptFinishOrder, orderBaseExample);

        //将没验票的改为“已过期”
        orderBaseCriteria.andIdNotIn(verifiedOrderIds);
        OOrder uptOverdueOrder = new OOrder();
        uptOverdueOrder.setStatus(OrderStatusEnum.OVERDUE.getCode());
        uptOverdueOrder.setModifyAt(new Date());
        uptOverdueOrder.setModifyBy("system");
        oOrderMapper.updateByExampleSelective(uptOverdueOrder, orderBaseExample);

        //保存日志
        for (OOrder order : oldOrderList) {
            OrderStatusEnum oldStatus = OrderStatusEnum.get(order.getStatus()).get();
            //有验票设置为完成，否则设置为过期
            if(verifiedOrderIds.contains(order.getId())){
                order.setStatus(OrderStatusEnum.FINISH.getCode());
                order.setModifyAt(new Date());
                order.setModifyBy("system");
                sysOperateOrderLog(order, oldStatus, "订单完成", "");
            }else{
                order.setStatus(OrderStatusEnum.OVERDUE.getCode());
                order.setModifyAt(new Date());
                order.setModifyBy("system");
                sysOperateOrderLog(order, oldStatus, "订单过期", "");
            }
        }
    }
